...
3. `function.Run(RunParameters{Grid, Inputs, BufferIds})` — encodes the command buffer, dispatches threads, blocks until complete. Safe for concurrent use. `function.RunBatch([]RunParameters)`, `function.RunAsync(...)`, and `function.RunBatchAsync([]RunParameters)` (all paired with `RunHandle.Wait()` for the async ones) are throughput-oriented variants (see below).
...
- **Dispatch path depends on GPU family** — the dispatch uses `dispatchThreads:threadsPerThreadgroup:` (exact, no over-dispatch) only when the device reports non-uniform threadgroup support (`MTLGPUFamilyApple4`/`MTLGPUFamilyMac2`), detected once in `function_cache_init`. On other hardware it falls back to `dispatchThreadgroups:threadsPerThreadgroup:` with a rounded-up threadgroup count, and kernels must bounds-check their thread position. The threadgroup height is clamped to ≥1 to avoid an invalid zero-dimension threadgroup for high-register-pressure pipelines.
- **Four dispatch paths share two encode helpers** — `encode_dispatch` (static, in `Function.m`) does one dispatch: function lookup, pipeline set, argument binding, threadgroup sizing, `dispatchThreads`. `encode_batch_into` loops `encode_dispatch` over N dispatches into one command buffer. The four C entry points differ only in command-buffer management: `function_run` (single, commit, `waitUntilCompleted`); `function_run_batch` (`encode_batch_into`, single commit + wait — amortizes setup and the one CPU/GPU sync, per Apple's "minimize command buffers" guidance); `function_run_async` (single, commit, return the command buffer to Go as an opaque handle via `__bridge_retained`, no wait); `function_run_batch_async` (`encode_batch_into`, commit, return handle, no wait). All async paths pair with `function_wait` (`__bridge_transfer` back, `waitUntilCompleted`, check `MTLCommandBufferStatusError`). The handle is retained exactly once and released exactly once by `function_wait`; the Go `RunHandle` zeroes its pointer after `Wait` so a second `Wait` cannot double-free. Because an async batch is one command buffer, a single `Wait` completes the whole batch.
- **Batch pointer pinning** — `RunBatch`/`RunBatchAsync` share `marshalBatch`, which builds Go slices of `*C.float`/`*C.int` pointing into each `RunParameters`' backing arrays and passes those slices to C by address. cgo forbids handing C a Go pointer to memory containing other Go pointers, so each inner pointer is pinned with a caller-owned `runtime.Pinner` for the call (pinning also keeps the backing arrays alive, replacing the `runtime.KeepAlive` the single-dispatch `Run` uses). The cgo pointer checker (on by default under `go test`) will panic if this is gotten wrong, so it is exercised by the batch tests.
- **Async buffer lifetime** — `RunAsync`/`RunBatchAsync` copy `Inputs` during the call (`setBytes`), so they need not outlive it, but the `BufferIds` buffers are read/written by the GPU after the call returns and must stay open until `Wait`. They are held alive by the buffer cache's strong reference; closing a buffer before `Wait` is a caller bug. The encode (and thus the pinning) is fully synchronous within the call, so the `Pinner` is unpinned when the call returns even on the async paths.

...
func Test_toSlice(t *testing.T) {
	...
	var i int
	intSlice := toSlice[int](unsafe.Pointer(&i), 1)
	require.Equal(t, len(intSlice), 1)
	require.Equal(t, cap(intSlice), 1)
	require.Equal(t, 0, intSlice[0])
	i = 1
	require.Equal(t, 1, intSlice[0])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(intSlice)
	})

	var f float32
	floatSlice := toSlice[float32](unsafe.Pointer(&f), 1)
	require.Equal(t, len(floatSlice), 1)
	require.Equal(t, cap(floatSlice), 1)
	require.Equal(t, float32(0), floatSlice[0])
	f = 1
	require.Equal(t, float32(1), floatSlice[0])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(floatSlice)
	})

	var s string
	stringSlice := toSlice[string](unsafe.Pointer(&s), 1)
	require.Equal(t, len(stringSlice), 1)
	require.Equal(t, cap(stringSlice), 1)
	require.Equal(t, "", stringSlice[0])
	s = "test"
	require.Equal(t, "test", stringSlice[0])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(stringSlice)
	})

	var ss []string
	sliceSlice := toSlice[[]string](unsafe.Pointer(&ss), 1)
	require.Equal(t, len(sliceSlice), 1)
	require.Equal(t, cap(sliceSlice), 1)
	require.Equal(t, []string(nil), sliceSlice[0])
	ss = append(ss, "test")
	require.Equal(t, []string([]string{"test"}), sliceSlice[0])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(sliceSlice)
	})
	...
	var ms MyStruct
	structSlice := toSlice[MyStruct](unsafe.Pointer(&ms), 1)
	require.Equal(t, len(structSlice), 1)
	require.Equal(t, cap(structSlice), 1)
	require.Equal(t, MyStruct{}, structSlice[0])
	ms.i = 1
	ms.s = "test"
	require.Equal(t, MyStruct{i: 1, s: "test"}, structSlice[0])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(structSlice)
	})

	is := []int{1, 2, 3}
	sliceSlice2 := toSlice[[]int](unsafe.Pointer(&is), 1)
	require.Equal(t, len(sliceSlice2), 1)
	require.Equal(t, cap(sliceSlice2), 1)
	require.Equal(t, []int{1, 2, 3}, sliceSlice2[0])
	for i := range is {
		is[i]++
	}
	require.Equal(t, []int{2, 3, 4}, sliceSlice2[0])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(sliceSlice2)
	})

	intSlice = toSlice[int](unsafe.Pointer(&is[0]), 3)
	require.Equal(t, len(intSlice), 3)
	require.Equal(t, cap(intSlice), 3)
	require.Equal(t, 3, intSlice[1])
	is[1] = 30
	require.Equal(t, 30, intSlice[1])
	require.NotPanics(t, func() {
		_ = fmt.Sprint(intSlice)
	})

	invalidSlice := toSlice[string](unsafe.Pointer(&i), 1)
	require.Equal(t, len(invalidSlice), 1)
	require.Equal(t, cap(invalidSlice), 1)
	require.Panics(t, func() {
		_ = fmt.Sprint(invalidSlice)
	})

	invalidSlice2 := toSlice[int](unsafe.Pointer(&i), 3)
	require.Equal(t, len(invalidSlice2), 3)
	require.Equal(t, cap(invalidSlice2), 3)
	require.NotPanics(t, func() {
		_ = fmt.Sprint(invalidSlice2[0])
